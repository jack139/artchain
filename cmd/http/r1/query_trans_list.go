...
/* 查询交易信息清单 */
func QueryTransList(ctx *fasthttp.RequestCtx) {
	log.Println("query_trans_list")
	...
	// 查询链上数据
	respData2, err := queryTransListPage(ctx, uint64(page), uint64(limit))
	if err!=nil{
...
// 查询链上数据, 返回 map
func queryTransListPage(ctx *fasthttp.RequestCtx, page uint64, limit uint64) (*[]interface{}, error) {
	// 获取 ctx 上下文
	...
	params := &transtypes.QueryAllTransactionRequest{
		Pagination: &pageReq,
	}
